...
<body>
    ...
    <div class="accordion">
        ...
        <div class="accordion_content">
        <form class="accordion_form" method="post">
        <label>Old Inventory ID: <br><input class="inputbox" type="text" name="oldInventoryIdUpdate" placeholder=" Old Inventory ID"></label><br>
    ...
    <table>
    	...
    	if(isset($_POST['search'])){
            ...
            $sql = "SELECT * FROM inventory WHERE CONCAT(`inventory_id`,`film_id`,`store_id`,`last_update`) LIKE '%".$valueToSearch."%' ORDER BY inventory_id ASC LIMIT 500";
    		$result = mysqli_query($conn,$sql);
    	}
        else{
            $sql = "SELECT * FROM inventory ORDER BY inventory_id ASC LIMIT 500";
    		$result = mysqli_query($conn,$sql);
    	}               
	...
	<?php
		if(isset($_POST['delete'])){
			$delete_id = $_POST['id'];
            mysqli_query($conn,"DELETE FROM inventory WHERE inventory_id = '$delete_id'");
			echo '<script type="text/javascript">'; 
        	echo 'alert("You have deleted the data.");';
            echo 'window.location= "https://hfyks3.mercury.nottingham.edu.my/sakila/inventory.php";';
        	echo '</script>'; 
        }
		...
		if(isset($_POST['insert'])){
			...
			$insert = "INSERT INTO `inventory`(`inventory_id`, `film_id`, `store_id`, `last_update`) VALUES ('$inventoryid','$filmid','$storeid','$lastupdate')";
        ...
        if(isset($_POST['update'])){
            $oldinventoryid = $_POST['oldInventoryIdUpdate'];
			$newinventoryid = $_POST['inventoryIdUpdate'];
			$newfilmid = $_POST['filmIdUpdate'];
			$newstoreid = $_POST['storeIdUpdate'];
			$newlastupdate = date("Y-m-d H:i:s",time());
			$update = "UPDATE `inventory` SET `inventory_id`='$newinventoryid',`film_id`='$newfilmid',`store_id`='$newstoreif',`last_update`='$newlastupdate' WHERE inventory_id = '$oldinventoryid'";
